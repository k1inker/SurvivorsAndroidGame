...
namespace NTC.Global.Pool
{
    public static class NightPool
    {
        ...
        /// </summary>
        public static void InstallPoolItems(PoolPreset poolPreset, DiContainer container)
        {
            ...
            foreach (var poolItem in poolPreset.PoolItems)
            {
                ...
                pool.PopulatePool(poolItem.Size, poolItem.diContanier, container);
            }
        }
        ...
        {
            return 
                DefaultSpawn(component.gameObject, position, rotation, null, false, null).
                GetComponent<T>();
        }

        public static T Spawn<T>(T component, Transform parent, DiContainer container = null, Quaternion rotation = default, 
            bool worldStaysPosition = false) where T : Component
        {
            var position = parent != null 
                ...
                : Vector3.zero;
            return 
                DefaultSpawn(component.gameObject, position, rotation, parent, worldStaysPosition,container).
                GetComponent<T>();
        }

        public static GameObject Spawn(GameObject toSpawn, DiContainer container = null, Vector3 position = default, Quaternion rotation = default)
        {
            return DefaultSpawn(toSpawn, position, rotation, null, false, container);
        }
        
        public static GameObject Spawn(GameObject toSpawn, DiContainer container, Transform parent, Quaternion rotation = default, 
            bool worldPositionStays = false)
        {
            ...
            return DefaultSpawn(toSpawn, position, rotation, parent, worldPositionStays, container);
        }
        ...
        private static GameObject DefaultSpawn(GameObject prefab, Vector3 position, Quaternion rotation, 
            Transform parent, bool worldPositionStays, DiContainer container)
        {
            if (IsEditor)
                return default;
            
            var pool = GetPoolByPrefab(prefab);
            var freePoolable = pool.GetFreeObject(container);
